...
export default function AdminSchedules() {
  ...
  const fetchSchedules = async () => {
    try {
      ...
      const schedulesRes = await adminService.getSchedules(requestParams);
      const items =
        schedulesRes.data.items ||
        schedulesRes.data.data ||
        schedulesRes.data ||
        [];
      setSchedules(items);
      setTotalRows(
        schedulesRes.data.paginate?.total ??
  ...
  const handleSubmit = async () => {
    try {
      if (formData.id) {
        ...
        await adminService.updateSchedule(formData.id, payload);

        if (remainingDays.length > 0) {
          const { id, ...baseData } = formData;
          await Promise.all(
            remainingDays.map((day) =>
              adminService.createSchedule({ ...baseData, day_of_week: day }),
            ),
          );
        }
          ...
          showNotification({
            message:
              remainingDays.length > 0
                ? `Cập nhật + thêm ${remainingDays.length} lịch thành công`
                : "Cập nhật thành công",
            severity: "success",
          }),
        );
        ...
        await Promise.all(
          days.map((day) =>
            adminService.createSchedule({ ...formData, day_of_week: day }),
          ),
        );
        dispatch(
          showNotification({
            message: `Thêm mới ${days.length} lịch thành công`,
            severity: "success",
          }),
        );
      }
    ...
    <Box>
          ...
          <TableBody>
            {schedules.map((row) => (
              <TableRow
                ...
                <TableCell>
                  <Chip
                    label={row.is_active ? "Hoạt động" : "Tắt"}
                    size="small"
                    sx={{
      ...
      <Dialog
        ...
        <DialogTitle
          ...
          {formData.id ? "Sửa lịch sân" : "Thêm lịch sân"}
        </DialogTitle>
        <DialogContent sx={{ pt: 3 }}>
          ...
          <FormControl fullWidth margin="dense">
            <InputLabel id="schedule-day-of-week-label">
              Thứ trong tuần (có thể chọn nhiều)
            </InputLabel>
            <Select
              labelId="schedule-day-of-week-label"
              multiple
              value={
